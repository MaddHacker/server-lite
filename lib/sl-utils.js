...
const _defaultOutput = new om.Out();
...
class slUtils {
    constructor(output) {
        this._out = output || _defaultOutput;
    }
            ...
            fs.access(filePath, fs.constants.R_OK, (accessErr) => {
                if (accessErr) { reject(this.errStr(accessErr)); }
                    ...
                    fs.readFile(filePath, (readErr, data) => {
                        (readErr) ? readError(this.errStr(readErr)) : fulfill(data);
                    });
                }
            });
        });
    ...
    respondWithFileFromPath(filePath, response) {
        this.loadDataFromFile(filePath).then((data) => {
            ...
            this._out.e(stringz.fmt('%{0} MSG: "%{1}"', msg, err));
            this.writeResponse(response, 500, con.text(msg));
        });
    }
