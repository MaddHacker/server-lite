...
class slServer {
    ...
    _start(svr) {
        ...
        out.i(stringz.fmt('Server started on port %{s}', this._config.listenOptions.port));
    }

    stop() {
        out.d('Stopping server...');
        this._server.close();
        out.i('Server has stopped.');
    }
}
...
class slHttpServer extends slServer {
    ...
    start() { super.start(http.createServer(this._config.onRequest)); }
}
...
class slHttpsServer extends slServer {
    ...
    start() { super.start(https.createServer(this._config.httpsOpts, this._config.onRequest)); }
}

module.exports = {
    slServer: slServer,
    slHttp: slHttpServer,
    slHttps: slHttpsServer
};
