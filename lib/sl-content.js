 ...
 */
module.exports = {
    binary: (content) => { return Content('application/octet-stream', content, binaryEncoding); },
    text: (content) => { return Content('text/plain', content, utf8Encoding); },
    byExtension: (extension, content) => {
        let contentObj = mimeTypes[extension] || mimeTypes['bin'];
        return new Content(contentObj.mimeType, content, contentObj.encoding);
    },
    custom: Content,
    mimeTypes: mimeTypes,
    typeString: (mimeTypeObj) => {
        let contentObj = mimeTypeObj || mimeTypes['bin'];
        return contentObj.mimeType + ((contentObj.encoding === utf8Encoding) ? utf8EncodingCharset : '');
    }
};
